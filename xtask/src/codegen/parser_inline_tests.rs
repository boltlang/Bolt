...
use crate::{
    codegen::{CodegenType, CommentBlock, ensure_file_contents, reformat},
    project_root,
    util::list_rust_files,
};

pub(crate) fn generate(check: bool) {
    let parser_crate_root = project_root().join("crates/boltlang-parser");
...
fn install_tests(tests: &HashMap<String, Test>, tests_dir: PathBuf, check: bool) -> Result<bool> {
    ...
    for (name, test) in tests {
        ...
        if ensure_file_contents(CodegenType::ParserTests, &path, &test.text, check) {
